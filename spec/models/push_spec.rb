...
RSpec.describe Push do
  describe 'validation' do
    let(:valid_assets) { create_list(:asset_resource, rand(2..4), validation_status_for_aapb: [AssetResource::VALIDATION_STATUSES[:valid]]) }
    ...
    context 'when the IDs exists, but some are missing children' do
      let(:asset_missing_children_1) { create(:asset_resource, validation_status_for_aapb: [AssetResource::VALIDATION_STATUSES[:missing_children]]) }
      let(:asset_missing_children_2) { create(:asset_resource, validation_status_for_aapb: [AssetResource::VALIDATION_STATUSES[:missing_children]]) }
    ...
    context 'when asset has not been validated' do
      let(:asset_status_not_validated) { create(:asset_resource, validation_status_for_aapb: [AssetResource::VALIDATION_STATUSES[:status_not_validated]]) }
