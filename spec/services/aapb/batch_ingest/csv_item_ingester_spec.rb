...
RSpec.describe AAPB::BatchIngest::CSVItemIngester do
  ...
  let(:multivalue_add_source_data) {
    "{\"Asset\":{\"id\":\"cpb-aacip_600-4746q1sh22\",\"sonyci_id\":[\"123456f\"],\"special_collection\":[\"Isn't that special 3.\"],\"asset_types\":[\"Segment\"],\"genre\":[\"Interview\"],\"spatial_coverage\":[\"Mars\"]}}"
  }
  ...
  describe '#ingest and overwrite Asset attributes' do
    context 'using aapb_csv_reader_3' do
      ...
      it 'updates an existing Asset' do
        ...
        expect(asset.admin_data.special_collection).to eq(['Collection1', 'Collection2'])
        update_asset
        asset.reload
        asset.admin_data.reload
        expect(asset.asset_types).to eq(['Album'])
        expect(asset.local_identifier).to eq(['wfreu5'])
        expect(asset.admin_data.sonyci_id).to eq(['wfreu6'])
        expect(asset.admin_data.special_collection).to eq(['Snowflake Collection'])
      end
    end
  end
  ...
  describe '#ingest and add to Asset multivalue attributes' do
    context 'using aapb_csv_reader_4' do
      ...
      it 'updates an existing Asset with new multivalue attributes' do
        ...
        asset.reload
        asset.admin_data.reload
        expect(asset.asset_types).to eq(['Clip', 'Promo', 'Segment'])
        expect(asset.genre).to eq(['Drama', 'Debate', 'Interview'])
        expect(asset.spatial_coverage).to eq(['TEST spatial_coverage', 'Mars'])
        expect(asset.admin_data.sonyci_id).to eq(['123456f', 'Sony-1', 'Sony-2'])
        expect(asset.admin_data.special_collection).to eq(["Isn't that special 3.", 'Collection1', 'Collection2',])
      end
    end
  end
end