...
RSpec.describe AMS::Export::Results::DigitalInstantiationsCSVResults do
  describe '#filepath' do
    ...
    let(:solr_docs) do
      assets.map { |asset| asset.digital_instantiations }.flatten
    end

    subject { described_class.new(solr_documents: solr_docs) }

    let(:expected_header) { AMS::CsvExportExtension.fields_for('digital_instantiation') }
    let(:expected_rows) { solr_docs.map { |solr_doc| solr_doc.csv_row_for('digital_instantiation') } }
