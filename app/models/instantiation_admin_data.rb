class InstantiationAdminData < ApplicationRecord
  self.table_name = "instantiation_admin_data"
  include ::EmptyDetection

  # Find the admin data associated with the Global Identifier (gid)
  # @param [String] gid - Global Identifier for this instantiation_admin_data (e.g.gid://ams/instantiationadmindata/1)
  ...
  def self.find_by_gid!(gid)
    ...
    raise ActiveRecord::RecordNotFound, "Couldn't find InstantiationAdminData matching GID #{gid}" unless result
    result
  end

  # Return the Global Identifier for this admin data.
  # @return [String] Global Identifier (gid) for this AdminData (e.g.gid://ams/instantiationadmindata/1)
  def gid
